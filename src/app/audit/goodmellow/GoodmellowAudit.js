...
const CATEGORY = {
  ...
  "Hero UX": { opacity: "opacity-70" },
  Ecommerce: { opacity: "opacity-55" },
  Trust: { opacity: "opacity-42" },
  "AOV/Sub": { opacity: "opacity-30" },
};

const SHORT_ROADMAP = {
  "Week 1–2": [
    "Product-led hero with dual CTAs",
    "Move promos after education",
    "Add education entry points",
    "Add dose, price and use-case copy",
    "Fix broken product links",
    "Surface review snippets near CTAs",
  ],
  "Week 3–6": [
    "Rebuild experience cards as shop cards",
    "Add cart drawer upsell",
    "Create bundle prompt",
    "Clarify PDP purchase summary",
    "Add founder and UGC proof",
    "Improve mobile buying flow",
  ],
  "Month 2–3": [
    "Measure with GA4, Shopify and Clarity",
    "A/B test hero and card structure",
    "Test offer and subscription modules",
    "Launch guided experience quiz",
    "Define retention opportunities",
  ],
};
  ...
  return (
    <Card className="flex flex-col gap-7 p-4 sm:p-6 md:p-9">
      ...
      <p className="max-w-[660px] font-general text-[0.98rem] leading-[1.7] text-white/62 md:text-base">
        Goodmellow already has the brand world. The conversion work is to make
        the buying decision clearer: what it is, which dose fits, why it works,
        and why to trust it before the CTA.
      </p>
        ...
        {benchmarkPatterns.map((item) => (
          ...
          >
            <div className="flex items-start justify-between gap-3">
              ...
              <ExternalLink size={12} className="mt-1 shrink-0 text-white/28" />
            </div>
    ...
    <Card className="flex flex-col gap-6 p-4 sm:p-6 md:p-9">
      ...
      <div className="grid grid-cols-3 gap-2.5">
        {[
          ...
          { value: 5, label: "Broken links", dataKey: "broken", accent: "text-white/72" },
        ...
        {roadmap.map((phase) => (
          ...
          >
            ...
            <ul className="flex flex-col gap-2.5">
              {(SHORT_ROADMAP[phase.phase] ?? phase.items).map((item) => (
                <li
                  key={item}
...
export default function GoodmellowAudit() {
  useSimpleRouteReady();
        ...
        <div className="mx-auto w-full max-w-[1720px] px-3 sm:px-4 md:px-[5%]">
          ...
          >
            ...
            <div className="flex flex-col items-start gap-4 md:items-end md:pb-2">
              <p className="max-w-[360px] font-general text-sm leading-[1.62] text-white/42 md:text-right">
                A compact visual cover for the CRO roadmap: product clarity,
                trust proof, buying confidence and validation.
              </p>
              <a
          ...
          <div ref={gridRef} className="flex flex-col gap-3 pt-3 md:gap-5 md:pt-5">
            ...
            <div className="bento-scroll">
              <AuditRowsCard
                label="04 — Homepage & Product Pages"
                title="Primary Ecommerce Friction"
